...
此前完整版服务的优化实验记录为 **20.217 秒 → 0.368 秒，约 55 倍**，每轮同样验证 2 万公式及结果。该实验的优化后程序与本公开本机版不是同一构建，服务入口和测评流程也不同；这组数值保留为原始优化实验，不能替代上方公开本机版的复测结果。实验 JSON 仅将本机绝对程序路径改为文件名，时延、校验字段和二进制哈希保持原值，并记录原文件哈希。
