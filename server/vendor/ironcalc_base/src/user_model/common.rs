...
use crate::{
    ...
    types::{
        ...
        SheetProperties, SheetState, Style, Table, Theme, VerticalAlignment,
    },
};
...
impl<'a> UserModel<'a> {
    ...
    pub fn get_iteration_settings(&self) -> crate::types::IterationSettings {
        self.model.get_iteration_settings()
    }

    /// Sets all iterative-calculation settings after validating their limits.
    pub fn set_iteration_settings(
        &mut self,
        settings: crate::types::IterationSettings,
    ) -> Result<(), String> {
        self.model.set_iteration_settings(settings)
    }
    ...
    pub fn set_iteration_options(
        ...
        self.model
            .set_iteration_options(enabled, maximum_iterations, maximum_change)
    }

    /// Returns the current structured-table registry.
    pub fn get_tables(&self) -> HashMap<String, Table> {
        self.model.workbook.tables.clone()
    }

    /// Replaces the structured-table registry and reparses formulas so
    /// structured references use the new names and ranges immediately.
    pub fn replace_tables(&mut self, tables: HashMap<String, Table>) {
        self.model.replace_tables(tables);
    }

    /// Translate a copied cell value from one coordinate to another using the
    /// same relative-reference rules as the clipboard implementation.
    pub fn extend_copied_value(
        ...
        self.model.extend_copied_value(value, source, target)
    }

    /// Translates a formula-bearing value with Excel cut/move semantics. References to cells
    /// inside `area` follow the moved range, while other relative references preserve their
    /// relationship to the formula anchor. This is also used by application sidecars such as
    /// data-validation formulas, which are not stored as ordinary worksheet cells.
    pub fn move_value_to_area(
        &mut self,
        value: &str,
        source: &CellReferenceIndex,
        target: &CellReferenceIndex,
        ...
        self.model
            .move_cell_value_to_area(value, source, target, area)
    }
    ...
    /// Number of user-visible undo units currently recorded by the engine.
    ///
    /// Embedders that keep additional workbook state outside IronCalc (for example
    /// native OOXML drawings or validation sidecars) use this monotonic depth to
    /// associate every request with the exact number of engine undo units it
    /// created.  The history contents remain private.
    pub fn undo_depth(&self) -> usize {
        self.history.undo_stack.len()
    }

    /// Number of user-visible redo units currently recorded by the engine.
    pub fn redo_depth(&self) -> usize {
        self.history.redo_stack.len()
    }

    /// Discards redo units after an application-side-only mutation.  A regular
    /// IronCalc diff already does this in `History::push`; native OOXML edits do
    /// not create an engine diff and therefore need the same branch semantics
    /// explicitly.
    pub fn discard_redo_history(&mut self) {
        self.history.redo_stack.clear();
    }

    /// Discards every user-visible undo and redo unit without changing the workbook.
    ///
    /// Embedders use this after constructing a pristine template in memory: the
    /// template becomes the baseline document, so its authoring operations must
    /// not be exposed as actions that the end user can undo.
    pub fn discard_all_history(&mut self) {
    ...
    ///
    /// This supports formulas that live outside cells, notably custom data
    /// validation formulas. Relative references use (`sheet`, `row`, `column`)
    /// as their origin and the returned value is the unformatted scalar result.
    pub fn evaluate_formula_value_at(
        ...
        row: i32,
        column: i32,
    ) -> Result<crate::cell::CellValue, String> {
        self.model
            .evaluate_formula_value_at(formula, sheet, row, column)
    }

    /// Evaluates an internally-stored English formula as a rectangular value array.
    ///
    /// Defined names, table structured references, dynamic spill references and regular ranges
    /// are expanded. Scalar formulas return a one-by-one array.
    pub fn evaluate_formula_values_at(
        ...
        row: i32,
        column: i32,
    ) -> Result<Vec<Vec<crate::cell::CellValue>>, String> {
        self.model
            .evaluate_formula_values_at(formula, sheet, row, column)
    }
    ...
    /// Stores the flattened value of an OOXML rich string without formula/number coercion.
    ///
    /// Rich-text run metadata is maintained by the embedding application. The
    /// flattened cell value still receives its own literal-text undo record so
    /// an application-level transaction can restore both layers atomically.
    pub fn set_rich_text_plain_value(
        ...
        self.model
            .set_rich_text_plain_value(sheet, row, column, value)?;
        self.evaluate_if_not_paused();
        self.push_diff_list(vec![Diff::SetRichTextPlainValue {
            sheet,
            row,
            column,
            new_value: value.to_string(),
    ...
    // ---- OpenCell patch: 合并单元格 API ----

    /// OpenCell patch: returns the merged-cell ranges of a sheet (e.g. `["A1:B2"]`).
    pub fn get_merged_cells(&self, sheet: u32) -> Result<Vec<String>, String> {
        Ok(self.model.workbook.worksheet(sheet)?.merge_cells.clone())
    }

    /// OpenCell patch: merges the given range keeping only the top-left content
    /// (Excel semantics). Overlapping merges are absorbed. Single undo unit.
    pub fn merge_cells_range(
        &mut self,
        sheet: u32,
        row: i32,
        column: i32,
        ...
        if !(is_valid_row(row)
            && is_valid_row(row1)
            && is_valid_column_number(column)
            && is_valid_column_number(column1))
        {
            return Err("Invalid range".to_string());
        }
        let (r0, r1) = (row.min(row1), row.max(row1));
        let (c0, c1) = (column.min(column1), column.max(column1));
        if r0 == r1 && c0 == c1 {
            return Err("Cannot merge a single cell".to_string());
        }
        // 1. 收集旧合并列表 + 区域旧单元格（用于单个 undo 单元恢复）
        let old_merge = self.model.workbook.worksheet(sheet)?.merge_cells.clone();
        let mut old_cells: Vec<Vec<Option<Cell>>> = Vec::new();
        for r in r0..=r1 {
            let mut line = Vec::new();
            for c in c0..=c1 {
                line.push(self.model.workbook.worksheet(sheet)?.cell(r, c).cloned());
            }
            old_cells.push(line);
        }
        // 2. 模型层操作（不 push diff）：吸收相交合并、清除非左上角、登记新合并
        {
            let ws = self.model.workbook.worksheet_mut(sheet)?;
            ws.merge_cells.retain(|m| match parse_merge_ref(m) {
                Some((mr0, mc0, mr1, mc1)) => mr1 < r0 || mr0 > r1 || mc1 < c0 || mc0 > c1,
                None => true,
            });
        }
        if c1 > c0 {
        ...
        let start = crate::expressions::utils::number_to_column(c0).ok_or("bad column")?;
        let end = crate::expressions::utils::number_to_column(c1).ok_or("bad column")?;
        let reference = format!("{start}{r0}:{end}{r1}");
        {
            let ws = self.model.workbook.worksheet_mut(sheet)?;
            if !ws.merge_cells.contains(&reference) {
                ws.merge_cells.push(reference);
            }
        }
        // 3. 单个 undo 单元：恢复内容 + 合并列表
        let new_merge = self.model.workbook.worksheet(sheet)?.merge_cells.clone();
        self.push_diff_list(vec![
    ...
    /// OpenCell patch: removes every merged range intersecting the given area.
    pub fn unmerge_cells_range(
        &mut self,
        sheet: u32,
        row: i32,
        column: i32,
...
// OpenCell patch: 解析 "A1:B2" 形式的合并引用 → (r0, c0, r1, c1)
fn parse_merge_ref(reference: &str) -> Option<(i32, i32, i32, i32)> {
...
fn parse_a1_cell(s: &str) -> Option<(i32, i32)> {
    ...
    let c = crate::expressions::utils::column_to_number(&col).ok()?;
