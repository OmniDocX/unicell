...
fn t_array_value(value: ArrayNode) -> ArrayNode {
    match value {
        ArrayNode::String(value) => ArrayNode::String(value),
        ArrayNode::Error(error) => ArrayNode::Error(error),
        ArrayNode::Number(_) | ArrayNode::Boolean(_) | ArrayNode::Empty => {
...
impl<'a> Model<'a> {
        ...
        for arg in args {
            match self.evaluate_node_in_context(arg, cell) {
                ...
                CalcResult::Array(array) => {
                    // OpenCell patch: CONCAT 拼接数组全部元素（Excel 语义）
                    for row in &array {
                        for node in row {
                            match node {
                                ArrayNode::String(s) => result = format!("{result}{s}"),
                                ArrayNode::Number(f) => result = format!("{result}{f}"),
                                ArrayNode::Boolean(b) => {
                                    result =
                                        format!("{result}{}", if *b { "TRUE" } else { "FALSE" })
                                }
                                ArrayNode::Error(e) => {
                                    return CalcResult::new_error(
                                        e.clone(),
                ...
                CalcResult::Lambda(_) => {
                    return CalcResult::Error {
                        error: Error::NIMPL,
                        origin: cell,
                        message: "Lambdas not supported here".to_string(),
                    }
                }
            };
        }
    ...
    pub(crate) fn fn_len(&mut self, args: &[Node], cell: CellReferenceIndex) -> CalcResult {
        ...
        match self.get_string_or_array(&args[0], cell) {
            Ok(crate::cast::StringOrArray::String(s)) => {
                CalcResult::Number(s.chars().count() as f64)
            }
            Ok(crate::cast::StringOrArray::Array(array)) => CalcResult::Array(
                array
                    .iter()
                    .map(|row| {
                        row.iter()
                            .map(|node| match array_node_to_string(node) {
                                Ok(s) => ArrayNode::Number(s.chars().count() as f64),
    ...
    pub(crate) fn fn_trim(&mut self, args: &[Node], cell: CellReferenceIndex) -> CalcResult {
        ...
        match self.get_string_or_array(&args[0], cell) {
            Ok(crate::cast::StringOrArray::String(s)) => CalcResult::String(trim_ascii_spaces(&s)),
            Ok(crate::cast::StringOrArray::Array(array)) => CalcResult::Array(
                array
                    .iter()
                    .map(|row| {
                        row.iter()
                            .map(|node| match array_node_to_string(node) {
                                Ok(s) => ArrayNode::String(trim_ascii_spaces(&s)),
                                Err(e) => ArrayNode::Error(e),
                            })
                            .collect()
                    })
    ...
    pub(crate) fn fn_unicode(&mut self, args: &[Node], cell: CellReferenceIndex) -> CalcResult {
        ...
        let unicode = |s: String| {
            s.chars()
                .next()
                .map(|c| ArrayNode::Number(c as u32 as f64))
                .unwrap_or(ArrayNode::Error(Error::VALUE))
        };
        match self.get_string_or_array(&args[0], cell) {
            Ok(crate::cast::StringOrArray::String(s)) => match unicode(s) {
                ArrayNode::Number(value) => CalcResult::Number(value),
                _ => CalcResult::new_error(Error::VALUE, cell, "Empty cell".to_string()),
            },
            Ok(crate::cast::StringOrArray::Array(array)) => CalcResult::Array(
                array
                    .iter()
    ...
    pub(crate) fn fn_textjoin(&mut self, args: &[Node], cell: CellReferenceIndex) -> CalcResult {
        ...
        for arg in &args[2..] {
            match self.evaluate_node_in_context(arg, cell) {
                ...
                CalcResult::EmptyArg => {}
                CalcResult::Array(array) => {
                    // OpenCell patch: TEXTJOIN 支持数组参数（如 UNIQUE/SEQUENCE 结果，Excel 语义）
                    for row in &array {
                        for node in row {
                            match node {
                                ArrayNode::String(s) => values.push(s.clone()),
                                ArrayNode::Number(f) => values.push(format!("{f}")),
                                ArrayNode::Boolean(b) => {
                                    values.push(if *b { "TRUE" } else { "FALSE" }.to_string())
                                }
                                ArrayNode::Error(e) => {
                                    return CalcResult::new_error(
                                        e.clone(),
            ...
            CalcResult::Range { left, right } => {
                let Some(reference) = implicit_intersection(&cell, &Range { left, right }) else {
                    return CalcResult::new_error(
                        Error::VALUE,
                        cell,
                        "VALUE range does not intersect the formula row or column".to_string(),
                    );
                };
                match self.evaluate_cell(reference) {
                    CalcResult::String(text) => {
                        let currencies = vec!["$", "\u{20ac}"];
                        match parse_formatted_number(&text, &currencies, self.locale) {
                            Ok((value, _)) => CalcResult::Number(value),
                            Err(_) => CalcResult::new_error(
                                Error::VALUE,
                                cell,
                    ...
                    error @ CalcResult::Error { .. } => error,
                    _ => CalcResult::new_error(Error::VALUE, cell, "Invalid number".to_string()),
                }
            }
            CalcResult::EmptyCell | CalcResult::EmptyArg => CalcResult::Number(0.0),
    ...
    pub(crate) fn fn_t(&mut self, args: &[Node], cell: CellReferenceIndex) -> CalcResult {
        ...
        match result {
            ...
            error @ CalcResult::Error { .. } => error,
            CalcResult::Range { left, right } => CalcResult::Array(
                self.evaluate_range(left, right)
                    .into_iter()
                    .map(|row| row.into_iter().map(t_array_value).collect())
                    .collect(),
            ),
            CalcResult::Array(values) => CalcResult::Array(
                values
                    .into_iter()
                    .map(|row| row.into_iter().map(t_array_value).collect())
                    .collect(),
