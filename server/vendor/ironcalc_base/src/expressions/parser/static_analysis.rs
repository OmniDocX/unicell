...
// SHEETS accepts zero arguments or one reference/range argument.  Unlike
// SHEET, the range must not be implicitly intersected because the function
// counts the worksheets spanned by the whole reference.
fn args_signature_sheets(arg_count: usize) -> Vec<Signature> {
    if arg_count == 0 {
        vec![]
    } else if arg_count == 1 {
        vec![Signature::Vector]
...
fn get_function_args_signature(kind: &Function, arg_count: usize) -> Vec<Signature> {
    match kind {
        ...
        Function::Groupby => {
            let signature = [
                Signature::Vector, // row_fields
                Signature::Vector, // values
                Signature::Vector, // function / eta-reduced LAMBDA
                Signature::Scalar, // field_headers
                Signature::Scalar, // total_depth
                Signature::Vector, // sort_order may be a vector
                Signature::Vector, // filter_array
                Signature::Scalar, // field_relationship
            ];
            if (3..=8).contains(&arg_count) {
                signature[..arg_count].to_vec()
            } else {
                vec![Signature::Error; arg_count]
            }
        }
        Function::Pivotby => {
            let signature = [
                Signature::Vector, // row_fields
                Signature::Vector, // col_fields
                Signature::Vector, // values
                Signature::Vector, // function / eta-reduced LAMBDA
                Signature::Scalar, // field_headers
                Signature::Scalar, // row_total_depth
                Signature::Vector, // row_sort_order
                Signature::Scalar, // col_total_depth
                Signature::Vector, // col_sort_order
                Signature::Vector, // filter_array
                Signature::Scalar, // relative_to
            ];
        ...
        Function::T => args_signature_one_vector(arg_count),
        ...
        Function::N => args_signature_one_vector(arg_count),
        Function::Sheets => args_signature_sheets(arg_count),
...
fn static_analysis_on_function(kind: &Function, args: &[Node]) -> StaticResult {
    match kind {
        ...
        // OpenCell patch: LEN 对数组参数溢出（与 LEFT/RIGHT 一致，Excel 语义）
        ...
        Function::T => scalar_arguments(args),
        ...
        Function::Unicode => scalar_arguments(args),
        ...
        Function::Sheets => StaticResult::Scalar,
