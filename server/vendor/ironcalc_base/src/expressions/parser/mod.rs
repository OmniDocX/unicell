    ...
    fn parse_primary(&mut self) -> Node {
        ...
        match next_token {
            ...
            TokenType::Range3D {
                ...
                let start_index = self.get_sheet_index_by_name(&sheet_start);
                let end_index = self.get_sheet_index_by_name(&sheet_end);
                let context = &self.context;
                let a1_mode = self.lexer.is_a1_mode();

                let left_row = if left.absolute_row || !a1_mode {
                    left.row
                } else {
                    left.row - context.row
                };
                let left_column = if left.absolute_column || !a1_mode {
                    left.column
                } else {
                    left.column - context.column
                };
                let right_row = if right.absolute_row || !a1_mode {
                    right.row
                } else {
                    right.row - context.row
                };
                let right_column = if right.absolute_column || !a1_mode {
                    right.column
                } else {
                    right.column - context.column
                };

                let left_node = match start_index {
                    Some(sheet_index) => Node::ReferenceKind {
                        sheet_name: Some(sheet_start),
                        sheet_index,
                ...
                let right_node = match end_index {
                    Some(sheet_index) => Node::ReferenceKind {
                        sheet_name: Some(sheet_end),
                        sheet_index,
