...
use crate::{
    ...
    constants::excel_column_width_to_pixels,
    ...
    pub(super) fn apply_undo_diff_list(&mut self, diff_list: &DiffList) -> Result<(), String> {
        ...
        for diff in diff_list.iter().rev() {
            match diff {
                ...
                Diff::DeleteColumns {
                    ...
                    for (i, col_data) in old_data.iter().enumerate() {
                        ...
                        if let Some(col) = &col_data.column {
                            let width = excel_column_width_to_pixels(col.width);
                            let style = col.style;
                ...
                // OpenCell patch: undo 合并单元格
                Diff::SetMergedCells {
                    sheet, old_value, ..
                } => {
                    self.model.workbook.worksheet_mut(*sheet)?.merge_cells = old_value.clone();
                }
        ...
        for diff in diff_list {
            match diff {
                ...
                Diff::SetRichTextPlainValue {
                    ...
                    needs_evaluation = true;
                    self.model
                        .set_rich_text_plain_value(*sheet, *row, *column, new_value)?;
                }
                Diff::SetArrayValue {
                    sheet,
                    row,
                ...
                // OpenCell patch: redo 合并单元格
                Diff::SetMergedCells {
                    sheet, new_value, ..
                } => {
                    self.model.workbook.worksheet_mut(*sheet)?.merge_cells = new_value.clone();
                }
                Diff::UpdateConditionalFormatting {
                    sheet,
                    index,
