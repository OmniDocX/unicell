...
#[test]
fn cross_sheet_cut_moves_cf_rule_formula_and_single_undo_restores_source() {
    let mut model = new_empty_user_model();
    ...
    assert!(model.get_conditional_formatting_list(0).unwrap().is_empty());
    let target = model.get_conditional_formatting_list(1).unwrap();
    assert_eq!(target.len(), 1);
    assert_eq!(target[0].range, "C3:C4");
    match &target[0].cf_rule {
        CfRule::Formula { formula, .. } => assert_eq!(formula, "=$C$3>0"),
...
fn cut_paste_cf_range_partial_overlap_unchanged() {
    ...
    // CF range is unchanged because a partial cut is rejected atomically.
    let list = model.get_conditional_formatting_list(0).unwrap();
    assert_eq!(list.len(), 1);
    assert_eq!(list[0].range, "A1:A5");
}
