...
pub(crate) enum Diff {
    ...
    /// A rich-string edit stores its flattened calculation value as literal text.
    /// It needs a distinct diff because replaying it through `set_user_input`
    /// would turn values such as `=literal` or `00123` into formulas/numbers.
    SetRichTextPlainValue {
        sheet: u32,
        row: i32,
        column: i32,
        new_value: String,
    ...
    // OpenCell patch: 合并单元格列表变更（merge/unmerge 可撤销）
    SetMergedCells {
        sheet: u32,
