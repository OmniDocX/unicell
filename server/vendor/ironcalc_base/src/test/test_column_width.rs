...
use crate::constants::{excel_column_width_to_pixels, DEFAULT_COLUMN_WIDTH};
...
fn test_column_width_lower_edge() {
    ...
    assert!(
        (worksheet.get_column_width(6).unwrap() - excel_column_width_to_pixels(10.0)).abs()
            < f64::EPSILON
    );
...
fn test_column_width_higher_edge() {
    ...
    let worksheet = model.workbook.worksheet(0).unwrap();
    assert!(
        (worksheet.get_column_width(15).unwrap() - excel_column_width_to_pixels(10.0)).abs()
            < f64::EPSILON
    );
