...
以 **Microsoft 365（Office 365）和 WPS Office** 为同类产品对标，致力于建设功能最完善、工程资料最完整的国产源码公开办公项目。该表述是发展目标；当前功能范围与证据见 [同类产品对照](docs/COMPARISON.zh-CN.md)。自有代码使用非商业源码许可，具体条件见下文。

## 性能测评

<!-- BENCHMARK:START -->
有界 SUM 范围优化已同步至公开本机版。同一台机器、相同 1 万行 CSV（10 万单元格、2 万公式），导入并计算耗时的中位数从 **20.217 秒降至 0.661 秒，速度约为此前的 30.6 倍**。每项预热 2 次、正式测量 7 次。
...
| 数据操作 | 排序、筛选、冻结窗格、条件格式、数据验证及支持的模拟分析 |
...
| 自动化 | 可选自定义 U AI 与 5 个本机 MCP 工具 |

公开本机版不包含 R2、云存储、共享链接、多人协作、统一账户和托管额度系统。
...
| 接入项 | 配置 |
| --- | --- |
| U AI | 复制 `.env.example` 为 `.env.local`，配置兼容 Chat Completions 的服务。所选上下文会发送至该服务，见 [自配本机 AI](docs/LOCAL_AI.md)。 |
...
| MCP | `/mcp` 接受本机 HTTP POST，使用 Cookie 工作簿会话，提供信息查询、单元格/区域读取、单元格写入与区域格式设置，见 [本机 MCP](docs/MCP.md)。 |
