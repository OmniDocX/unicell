...
impl<'a> Lexer<'a> {
                    ...
                    _ => {
                        if char.is_alphabetic() || char == '_' {
                            ...
                            if peek_char == Some('!') {
                                ...
                                if let Some(reference) = self.consume_unquoted_3d_reference(&name) {
                                    return reference;
                                }
                            } else if peek_char == Some('$') {
    ...
    fn consume_quoted_sheet_reference(&mut self) -> TokenType {
        ...
        // Excel quotes a sheet span as one unit when either endpoint needs
        // quoting: `'January Sales:March Sales'!A1`.
        if let Some((sheet_start, sheet_end)) = sheet_name.split_once(':') {
            if sheet_start.is_empty() || sheet_end.is_empty() {
                return TokenType::Illegal(
                    self.set_error("Invalid 3-D worksheet span", self.position),
                );
            }
            if self.next_token() != TokenType::Bang {
                return TokenType::Illegal(self.set_error("Expected '!'", self.position));
            }
            return self.consume_3d_range(sheet_start.to_string(), sheet_end.to_string());
        }
        if self.peek_char() == Some(':') {
            if let Some(reference) = self.consume_quoted_3d_reference(&sheet_name) {
                return reference;
            }
        }
        if self.next_token() != TokenType::Bang {
    ...
    fn consume_unquoted_3d_reference(&mut self, sheet_start: &str) -> Option<TokenType> {
        let saved = self.position;
        self.position += 1; // ':'
        let sheet_end = if self.peek_char() == Some('\'') {
            self.position += 1;
            self.consume_single_quote_string().ok()?
        } else {
            let name = self.consume_identifier();
            if name.is_empty() {
                self.position = saved;
        ...
        self.position += 1;
        Some(self.consume_3d_range(sheet_start.to_string(), sheet_end))
    }

    fn consume_quoted_3d_reference(&mut self, sheet_start: &str) -> Option<TokenType> {
        let saved = self.position;
        self.position += 1; // ':'
        let sheet_end = if self.peek_char() == Some('\'') {
            self.position += 1;
    ...
    fn consume_3d_range(&mut self, sheet_start: String, sheet_end: String) -> TokenType {
        let parsed = if self.mode == LexerMode::A1 {
