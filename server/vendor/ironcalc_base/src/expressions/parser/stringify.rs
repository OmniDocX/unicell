...
fn quote_3d_sheet_span(start: &str, end: &str) -> String {
    if quote_name(start) == start && quote_name(end) == end {
        format!("{start}:{end}")
    } else {
        format!(
            "'{}:{}'",
...
fn stringify_3d_range(
    ...
    let (
        Node::ReferenceKind {
            sheet_name: Some(start_name),
            sheet_index: start_index,
            row: start_row,
            column: start_column,
            absolute_row: start_absolute_row,
            absolute_column: start_absolute_column,
        },
        Node::ReferenceKind {
            sheet_name: Some(end_name),
            sheet_index: end_index,
            row: end_row,
            column: end_column,
            absolute_row: end_absolute_row,
            absolute_column: end_absolute_column,
        },
    ) = (left, right)
    else {
        return None;
    };
    if start_index == end_index && start_name.eq_ignore_ascii_case(end_name) {
        return None;
    }

    let no_sheet = None;
    let first = stringify_reference(
        context,
        displace_data,
        &Reference {
            sheet_name: &no_sheet,
    ...
    let sheets = quote_3d_sheet_span(start_name, end_name);
    if first == last {
        Some(format!("{sheets}!{first}"))
    } else {
        Some(format!("{sheets}!{first}:{last}"))
    }
}
...
fn stringify(
    ...
    match node {
        ...
        OpRangeKind { left, right } => {
            if let Some(reference) = stringify_3d_range(left, right, context, displace_data) {
                reference
            } else {
                format!(
                    "{}:{}",
