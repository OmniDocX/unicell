...
impl<'a> UserModel<'a> {
    ...
    /// Updates Applies To and, where supported by the executable rule type,
    /// `stopIfTrue` without rebuilding the rule's differential format.
    pub fn update_conditional_formatting_metadata(
        ...
        let old = self.model.update_conditional_formatting_metadata(
            sheet,
            index as usize,
            new_range,
            stop_if_true,
        ...
        self.push_diff_list(vec![Diff::UpdateConditionalFormatting {
            ...
            old_rule: Box::new(old.cf_rule),
            old_priority: old.priority,
            new_range: stored.range,
            new_rule: Box::new(stored.cf_rule),
        }]);
        self.evaluate_if_not_paused();
        Ok(())
    }

    /// Duplicates a semantic CF rule while sharing its existing DXF. The model
    /// history records a regular add, so undo/redo remains exact.
    pub fn duplicate_conditional_formatting(
        ...
        let (priority, rule) =
            self.model
                .duplicate_conditional_formatting(sheet, index as usize, new_range)?;
        self.push_diff_list(vec![Diff::AddConditionalFormatting {
            sheet,
