...
UniCell 自有应用代码采用顶层 [PolyForm Noncommercial 1.0.0](../LICENSE)。符合条款的非商业使用免费，超出标准许可允许范围的商业用途须另行申请付费书面授权。具体申请方式见 [商业授权](COMMERCIAL_LICENSE.md)。

第三方组件保持各自许可，顶层条款不改变它们已有的授权。IronCalc / ironcalc_base 为 MIT OR Apache-2.0；KaTeX 为 MIT；其他依赖与可选工具见 [第三方声明](../THIRD_PARTY_NOTICES.md)。
