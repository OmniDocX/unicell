...
/// OpenCell patch: Excel semantics — in scalar context an array collapses to its
/// top-left element (e.g. `=LEN(UNIQUE(...))`, `=TEXT(SEQUENCE(2),..)`).
fn array_top_left(array: &[Vec<ArrayNode>], cell: CellReferenceIndex) -> CalcResult {
    match array.first().and_then(|row| row.first()) {
        Some(ArrayNode::Number(f)) => CalcResult::Number(*f),
        Some(ArrayNode::String(s)) => CalcResult::String(s.clone()),
        Some(ArrayNode::Boolean(b)) => CalcResult::Boolean(*b),
        Some(ArrayNode::Error(e)) => {
            CalcResult::new_error(e.clone(), cell, "Error in array".to_string())
        }
        Some(ArrayNode::Empty) => CalcResult::EmptyCell,
        None => CalcResult::new_error(Error::VALUE, cell, "Empty array".to_string()),
    }
}
    ...
    pub(crate) fn cast_to_number(
        ...
        match result {
            ...
            CalcResult::Range { left, right } => {
                ...
                let Some(reference) = implicit_intersection(&cell, &range) else {
                    return Err(CalcResult::new_error(
                        Error::VALUE,
                        cell,
                        "Range does not have an implicit intersection".to_string(),
                    ));
                };
                let value = self.evaluate_cell(reference);
                self.cast_to_number(value, cell)
            }
            // OpenCell patch: 标量上下文取数组左上角元素（Excel 语义）
            CalcResult::Array(array) => {
                let tl = array_top_left(&array, cell);
                self.cast_to_number(tl, cell)
            }
            CalcResult::Lambda(_) => Err(CalcResult::new_error(
                Error::VALUE,
                cell,
    ...
    pub(crate) fn cast_to_string(
        ...
        match result {
            ...
            CalcResult::Range { left, right } => {
                ...
                let value = self.evaluate_cell(reference);
                self.cast_to_string(value, cell)
            }
            // OpenCell patch: 标量上下文取数组左上角元素（Excel 语义）
            CalcResult::Array(array) => {
                let tl = array_top_left(&array, cell);
                self.cast_to_string(tl, cell)
            }
            CalcResult::Lambda(_) => Err(CalcResult::new_error(
                Error::VALUE,
                cell,
                "LAMBDA cannot be used as text".to_string(),
            )),
        }
    }
    ...
    pub(crate) fn cast_to_bool(
        ...
        match result {
            ...
            CalcResult::Range { left, right } => {
                ...
                let value = self.evaluate_cell(reference);
                self.cast_to_bool(value, cell)
            }
            // OpenCell patch: 标量上下文取数组左上角元素（Excel 语义）
            CalcResult::Array(array) => {
                let tl = array_top_left(&array, cell);
                self.cast_to_bool(tl, cell)
            }
            CalcResult::Lambda(_) => Err(CalcResult::new_error(
                Error::VALUE,
                cell,
                "LAMBDA cannot be used as a Boolean".to_string(),
            )),
        }
    }
