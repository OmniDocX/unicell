...
fn copy_non_anchor_part_of_array() {
    ...
    model
        .paste_from_clipboard(0, (3, 1, 5, 1), &cp.data, false)
        .unwrap();
...
// Cutting only spill children would silently move styles while leaving the owning formula at
// the source. Excel treats that as an indivisible spill boundary, so reject it atomically.
#[test]
fn cut_non_anchor_part_of_array() {
    ...
    // Destination C3:C5 keeps both values and styles untouched.
    assert!(!model.get_cell_style(0, 3, 3).unwrap().font.b);
    assert!(!model.get_cell_style(0, 4, 3).unwrap().font.b);
    assert!(!model.get_cell_style(0, 5, 3).unwrap().font.b);
...
//   2. Write "X" in C3; cut C3, paste onto A3 (a spill child)
//      → reject atomically because only the spill anchor owns the generated range
#[test]
fn paste_onto_spill_cell_then_undo_restores_spill() {
    ...
    assert_eq!(model.get_formatted_cell_value(0, 3, 1).unwrap(), "3");
    assert_eq!(model.get_formatted_cell_value(0, 6, 1).unwrap(), "6");
    // The cut source is untouched because no history unit was committed.
...
fn paste_from_cell_beyond_dimension_clears_style() {
    ...
    assert_eq!(style.fill.color, Color::Rgb("#00FF00".to_owned()));
