...
// ---------------------------------------------------------------------------
// Formula thresholds use the top-left of sqref as their parse anchor, then
// move relative references for every target cell (matching Excel).
// ---------------------------------------------------------------------------

#[test]
fn relative_formula_threshold_moves_for_each_target_row() {
    let mut model = new_empty_model();
    for (row, value, limit) in [(1, 5, 10), (2, 15, 10), (3, 12, 20)] {
        model.set_user_input(0, row, 1, value.to_string()).unwrap();
        model.set_user_input(0, row, 2, limit.to_string()).unwrap();
    }
    model.evaluate();
    model
        .add_conditional_formatting(0, "A1:A3", cell_is(ValueOperator::GreaterThan, "B1", None))
        .unwrap();
    model.evaluate();

    assert!(!is_red(&model, 1), "A1=5 is not greater than B1=10");
    assert!(is_red(&model, 2), "A2=15 is greater than B2=10");
    assert!(
        !is_red(&model, 3),
        "A3=12 must compare with B3=20, not the rule anchor B1"
    );
}

#[test]
fn absolute_formula_threshold_stays_fixed_for_each_target_row() {
    let mut model = new_empty_model();
    model.set_user_input(0, 1, 2, "10".to_string()).unwrap();
    for (row, value) in [(1, 5), (2, 15), (3, 12)] {
        model.set_user_input(0, row, 1, value.to_string()).unwrap();
    }
    model.evaluate();
    model
        .add_conditional_formatting(
            0,
            "A1:A3",
            cell_is(ValueOperator::GreaterThan, "$B$1", None),
        )
        .unwrap();
    model.evaluate();

    assert!(!is_red(&model, 1));
    assert!(is_red(&model, 2));
    assert!(is_red(&model, 3));
}

#[test]
fn cell_is_equal_compares_text_case_insensitively_and_moves_reference() {
    let mut model = new_empty_model();
    for (row, value, expected) in [
        ...
        model.set_user_input(0, row, 1, value.to_string()).unwrap();
        model
            .set_user_input(0, row, 2, expected.to_string())
            .unwrap();
    }
    model.evaluate();
    model
        .add_conditional_formatting(0, "A1:A3", cell_is(ValueOperator::Equal, "B1", None))
        .unwrap();
    model.evaluate();

    assert!(
        is_red(&model, 1),
        "text equality follows Excel casing rules"
    );
    assert!(!is_red(&model, 2));
    assert!(!is_red(&model, 3), "A3 must compare with B3, not B1");
}

#[test]
fn cell_is_equal_compares_logical_values_without_numeric_coercion() {
    let mut model = new_empty_model();
    model.set_user_input(0, 1, 1, "TRUE".to_string()).unwrap();
    model.set_user_input(0, 2, 1, "FALSE".to_string()).unwrap();
    model.evaluate();
    model
        .add_conditional_formatting(0, "A1:A2", cell_is(ValueOperator::Equal, "TRUE()", None))
        .unwrap();
    model.evaluate();
...
#[test]
fn empty_cell_does_not_sort_above_a_numeric_threshold() {
    let mut model = new_empty_model();
    model
        .add_conditional_formatting(0, "A1", cell_is(ValueOperator::GreaterThan, "3", None))
        .unwrap();
    model.evaluate();

    assert!(
        !is_red(&model, 1),
        "an empty cell must not match a numeric greater-than rule"
    );
}

#[test]
fn between_formula_bounds_move_together() {
    let mut model = new_empty_model();
    for (row, value, low, high) in [(1, 5, 1, 10), (2, 15, 20, 30), (3, 25, 20, 30)] {
        for (column, input) in [(1, value), (2, low), (3, high)] {
            model
    ...
    model
        .add_conditional_formatting(
            0,
            "A1:A3",
            cell_is(ValueOperator::Between, "B1", Some("C1")),
        )
        .unwrap();
    model.evaluate();
