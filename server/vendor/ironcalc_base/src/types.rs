...
/// A formatting run inside an OOXML shared or inline string.
///
/// Cell styles are not sufficient for rich text: a single cell may contain a red bold
/// heading followed by smaller black body copy. Importers can retain these runs alongside
/// the plain shared-string value while the calculation engine continues to operate on text.
#[derive(Serialize, Deserialize, Encode, Decode, Debug, PartialEq, Clone, Default)]
#[serde(default)]
...
/// Excel-compatible iterative calculation settings for circular formulas.
/// Iteration is opt-in so existing workbooks retain the `#CIRC!` behavior.
#[derive(Serialize, Deserialize, Encode, Decode, Debug, PartialEq, Clone)]
#[serde(default)]
pub struct IterationSettings {
    /// Whether circular formulas are solved iteratively.
    pub enabled: bool,
    /// Maximum number of workbook calculation passes.
    pub maximum_iterations: u32,
    /// Numeric convergence threshold between successive passes.
...
mod test {
    ...
    #[test]
    fn spreadsheet_theme_indices_map_before_tint() {
        // SpreadsheetML's numeric theme order is lt1, dk1, lt2, dk2, accent1..6,
        // hlink, folHlink (not the physical dk1-first order in theme1.xml).
        let theme = Theme {
            name: "Office 2007".to_string(),
            dk1: "#000000".to_string(),
            lt1: "#FFFFFF".to_string(),
        ...
        assert_eq!(theme.resolve(0, 0.0), "#FFFFFF");
        assert_eq!(theme.resolve(1, 0.0), "#000000");
        assert_eq!(theme.resolve(3, 0.0), "#1F497D");
        assert_eq!(theme.resolve(10, 0.0), "#0000FF");
        assert_eq!(theme.resolve(4, 0.8), "#DCE6F1");
    }
}
