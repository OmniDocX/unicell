...
class PublicBoundaryTests(unittest.TestCase):
    ...
    def test_hosted_components_cannot_reenter_the_public_index(self):
        names = ["crates/unippt-gateway/src/main.rs", "web/production_runtime.js",
                 "crates/unippt-server/src/gateway_guard.rs", "tools/mcp_workspace_tools.cjs",
                 "tools/unippt_mcp_remote.cjs", "plugins/unippt-chatgpt/plugin.json",
                 "docs/UNIPPT_OMNIDOC_R2_INTEGRATION.md", "docs/MCP_OAUTH.md"]
        for name in names:
            self.put(name, b"")
        self.assertEqual(len(scan_index(self.root)), len(names))
