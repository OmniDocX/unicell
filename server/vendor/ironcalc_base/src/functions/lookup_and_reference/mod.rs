...
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
enum R1C1Reference {
    Cell { row: R1C1Axis, column: R1C1Axis },
    Row(R1C1Axis),
    Column(R1C1Axis),
}

fn parse_r1c1_axis(text: &[u8], position: &mut usize) -> Result<R1C1Axis, String> {
    if *position >= text.len() {
        return Ok(R1C1Axis::Current);
    }
    if text[*position] == b'[' {
        *position += 1;
        let start = *position;
        if *position < text.len() && matches!(text[*position], b'+' | b'-') {
            *position += 1;
        }
        let digits = *position;
        ...
        if digits == *position || *position >= text.len() || text[*position] != b']' {
            return Err("Invalid relative R1C1 reference".to_string());
        }
        let value = std::str::from_utf8(&text[start..*position])
            .ok()
            .and_then(|value| value.parse::<i32>().ok())
            .ok_or_else(|| "R1C1 offset is outside the supported range".to_string())?;
        *position += 1;
        return Ok(R1C1Axis::Relative(value));
    }
    ...
    if start == *position {
        ...
        let value = std::str::from_utf8(&text[start..*position])
            ...
            .ok_or_else(|| "Absolute R1C1 indexes start at one".to_string())?;
        Ok(R1C1Axis::Absolute(value))
    }
}

fn parse_r1c1_endpoint(text: &str) -> Result<R1C1Reference, String> {
    let upper = text.trim().to_ascii_uppercase();
    let bytes = upper.as_bytes();
    if bytes.is_empty() {
        return Err("Empty R1C1 reference".to_string());
    }
    let mut position = 1;
    match bytes[0] {
        b'R' => {
            let row = parse_r1c1_axis(bytes, &mut position)?;
            if position == bytes.len() {
                return Ok(R1C1Reference::Row(row));
            }
            if bytes[position] != b'C' {
                return Err("Expected C in R1C1 reference".to_string());
            }
            position += 1;
            let column = parse_r1c1_axis(bytes, &mut position)?;
            if position != bytes.len() {
                return Err("Unexpected text after R1C1 reference".to_string());
            }
            Ok(R1C1Reference::Cell { row, column })
        }
        b'C' => {
            let column = parse_r1c1_axis(bytes, &mut position)?;
            if position != bytes.len() {
                return Err("Unexpected text after R1C1 column reference".to_string());
            }
            Ok(R1C1Reference::Column(column))
        }
        _ => Err("R1C1 reference must start with R or C".to_string()),
    }
}

fn resolve_r1c1_axis(axis: R1C1Axis, anchor: i32, maximum: i32) -> Result<i32, String> {
    let value = match axis {
        R1C1Axis::Current => anchor,
        R1C1Axis::Absolute(value) => value,
        R1C1Axis::Relative(offset) => anchor
            .checked_add(offset)
            .ok_or_else(|| "R1C1 reference is outside the worksheet".to_string())?,
    };
    if !(1..=maximum).contains(&value) {
        return Err("R1C1 reference is outside the worksheet".to_string());
    }
    Ok(value)
}

/// Separates an optional (possibly quoted) sheet name from an R1C1 reference.
/// Apostrophes escaped as `''` are unescaped using Excel's sheet-name rules.
fn split_r1c1_sheet(reference: &str) -> Result<(Option<String>, String), String> {
    ...
    while position < bytes.len() {
        if bytes[position] == b'\'' {
            if quoted && position + 1 < bytes.len() && bytes[position + 1] == b'\'' {
                position += 2;
                continue;
            }
            quoted = !quoted;
        } else if bytes[position] == b'!' && !quoted {
            if bang.replace(position).is_some() {
                return Err("Invalid sheet-qualified R1C1 reference".to_string());
            }
        }
        position += 1;
    }
    if quoted {
        return Err("Unterminated quoted sheet name".to_string());
    }
    let Some(bang) = bang else {
        return Ok((None, reference.to_string()));
    };
    let sheet_text = reference[..bang].trim();
    let reference_text = reference[bang + 1..].trim();
    if sheet_text.is_empty() || reference_text.is_empty() || sheet_text.contains(['[', ']']) {
        return Err("Invalid sheet-qualified R1C1 reference".to_string());
    }
    let sheet = if sheet_text.starts_with('\'') && sheet_text.ends_with('\'') {
        if sheet_text.len() < 2 {
            return Err("Invalid quoted sheet name".to_string());
        }
        sheet_text[1..sheet_text.len() - 1].replace("''", "'")
    } else {
        sheet_text.to_string()
    };
    Ok((Some(sheet), reference_text.to_string()))
}

fn parse_indirect_r1c1<F>(
    reference: &str,
...
where
    F: Fn(&str) -> Option<u32>,
{
    let (sheet_name, reference) = split_r1c1_sheet(reference)?;
    let sheet = match sheet_name {
        Some(name) => get_sheet_index_by_name(&name)
            .ok_or_else(|| format!("Sheet \"{name}\" could not be found"))?,
        None => anchor.sheet,
    };
    let mut parts = reference.split(':');
    let left = parse_r1c1_endpoint(parts.next().unwrap_or_default())?;
    let right = parts.next().map(parse_r1c1_endpoint).transpose()?;
    if parts.next().is_some() {
        return Err("R1C1 reference contains more than one range separator".to_string());
    }

    let resolve_cell = |row: R1C1Axis, column: R1C1Axis| -> Result<CellReferenceIndex, String> {
        Ok(CellReferenceIndex {
            sheet,
            row: resolve_r1c1_axis(row, anchor.row, LAST_ROW)?,
            column: resolve_r1c1_axis(column, anchor.column, LAST_COLUMN)?,
        })
    };
    match (left, right) {
        (R1C1Reference::Cell { row, column }, None) => {
            Ok(ParsedReference::CellReference(resolve_cell(row, column)?))
        }
        (
            R1C1Reference::Cell {
                row: left_row,
            ...
            Some(R1C1Reference::Cell {
                row: right_row,
                column: right_column,
            }),
        ) => {
            let left = resolve_cell(left_row, left_column)?;
            let right = resolve_cell(right_row, right_column)?;
            Ok(ParsedReference::Range(
                CellReferenceIndex {
                    sheet,
                    row: left.row.min(right.row),
                    column: left.column.min(right.column),
                },
                CellReferenceIndex {
                    sheet,
                    row: left.row.max(right.row),
                    column: left.column.max(right.column),
                },
            ))
        }
        (R1C1Reference::Row(left), right) => {
            let left = resolve_r1c1_axis(left, anchor.row, LAST_ROW)?;
            let right = match right {
                Some(R1C1Reference::Row(right)) => resolve_r1c1_axis(right, anchor.row, LAST_ROW)?,
                None => left,
                _ => return Err("Cannot mix row, column and cell R1C1 references".to_string()),
            };
            Ok(ParsedReference::Range(
                CellReferenceIndex {
                    sheet,
        ...
        (R1C1Reference::Column(left), right) => {
            let left = resolve_r1c1_axis(left, anchor.column, LAST_COLUMN)?;
            let right = match right {
                Some(R1C1Reference::Column(right)) => {
                    resolve_r1c1_axis(right, anchor.column, LAST_COLUMN)?
                }
                None => left,
                _ => return Err("Cannot mix row, column and cell R1C1 references".to_string()),
            };
            Ok(ParsedReference::Range(
                CellReferenceIndex {
                    sheet,
                    row: 1,
                    column: left.min(right),
                },
                CellReferenceIndex {
                    sheet,
                    row: LAST_ROW,
                    column: left.max(right),
                },
            ))
        }
        _ => Err("Cannot mix row, column and cell R1C1 references".to_string()),
    }
}
...
impl<'a> Model<'a> {
    ...
    pub(crate) fn fn_indirect(&mut self, args: &[Node], cell: CellReferenceIndex) -> CalcResult {
        ...
        match value {
            Ok(s) => {
                // A textual defined name is a reference independently of the A1/R1C1
                // switch. Prefer a sheet-local name, matching Excel's name resolution.
                let name = s.trim().to_lowercase();
                if let Some(defined_name) = self
                    .parsed_defined_names
                    .get(&(Some(cell.sheet), name.clone()))
                    .or_else(|| self.parsed_defined_names.get(&(None, name)))
                    .cloned()
                {
                    return match defined_name {
                        crate::model::ParsedDefinedName::CellReference(reference) => {
                            CalcResult::Range {
                                left: reference,
                                right: reference,
                            }
                        }
                        crate::model::ParsedDefinedName::RangeReference(range) => {
                            CalcResult::Range {
                                left: range.left,
                                right: range.right,
                            }
                        ...
                        _ => CalcResult::Error {
                            error: Error::REF,
                            origin: cell,
                            message: "Defined name is not a reference".to_string(),
                        },
                    };
                }
                ...
                let parsed_reference = if a1 {
                    ...
                    parse_indirect_r1c1(&s, cell, |name| self.get_sheet_index_by_name(name))
                };
        ...
        if let CalcResult::Range { left, right } = self.evaluate_node_with_reference(&args[0], cell)
        {
            if left.sheet != right.sheet {
                return CalcResult::Error {
                    error: Error::VALUE,
                    origin: cell,
                    message: "FORMULATEXT does not accept a 3-D reference".to_string(),
                };
            }
            // Excel specifies the upper-left cell for a multi-cell reference; this is
            // deliberately not implicit intersection with the formula's own position.
