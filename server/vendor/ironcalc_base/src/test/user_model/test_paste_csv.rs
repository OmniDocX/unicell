...
#[test]
fn cross_sheet_cut_retargets_observers_and_defined_names_and_undoes_once() {
    let mut model = new_empty_user_model();
    model.new_sheet().unwrap();
    model.set_user_input(0, 1, 1, "42").unwrap();
    ...
    model
        .new_defined_name("MovedGlobal", None, "Sheet1!$A$1")
        .unwrap();
    model
        .new_defined_name("MovedLocal", Some(0), "Sheet1!$A$1")
        .unwrap();
    ...
    assert_eq!(model.get_cell_content(1, 4, 4).unwrap(), "42");
    assert!(model.get_cell_content(0, 1, 2).unwrap().contains("Sheet2!D4"));
    assert!(model.get_cell_content(1, 1, 1).unwrap().contains("D4"));
    let names = model.get_defined_name_list();
    assert!(names.contains(&(
        "MovedGlobal".to_string(),
        None,
        "Sheet2!$D$4".to_string()
    )));
    assert!(names.contains(&(
    ...
    model.undo().unwrap();
    assert_eq!(model.get_cell_content(0, 1, 1).unwrap(), "42");
    assert_eq!(model.get_cell_content(1, 4, 4).unwrap(), "");
    assert_eq!(model.get_cell_content(0, 1, 2).unwrap(), "=A1*2");
    assert_eq!(model.get_cell_content(1, 1, 1).unwrap(), "=Sheet1!A1+1");
