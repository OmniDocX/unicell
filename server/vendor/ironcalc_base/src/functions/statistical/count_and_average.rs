...
impl<'a> Model<'a> {
            ...
            match self.evaluate_node_in_context(arg, cell) {
                ...
                CalcResult::Range { left, right } => {
                    for sheet in left.sheet.min(right.sheet)..=left.sheet.max(right.sheet) {
                        for row in left.row.min(right.row)..=left.row.max(right.row) {
                            for column in
            ...
            match self.evaluate_node_in_context(arg, cell) {
                ...
                CalcResult::String(value) if !matches!(arg, Node::ReferenceKind { .. }) => {
                    if let Some(parsed) = self.cast_number(&value) {
                        ...
                        // The A-family treats text as zero, including a literal
                        // text argument (AVERAGEA("text") == 0).
                        f(0.0);
                    }
                }
                CalcResult::Array(array) => {
    ...
    pub(crate) fn fn_averagea(&mut self, args: &[Node], cell: CellReferenceIndex) -> CalcResult {
        ...
        let mut count = 0.0;
        let mut sum = 0.0;
        if let Err(error) = self.for_each_value_a(args, cell, |value| {
            count += 1.0;
            sum += value;
        }) {
            return error;
