...
pub(crate) fn implicit_intersection(
    ...
    // A 3-D range cannot be reduced to a scalar.  A range on another sheet can:
    // implicit intersection uses the formula cell's row/column, but the value is
    // read from the range's sheet (for example a name referring to Sheet2!A:A).
