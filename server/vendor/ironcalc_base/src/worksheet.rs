...
impl Worksheet {
    ...
    pub fn set_style(&mut self, style_index: i32) -> Result<(), String> {
        self.cols = vec![Col {
            ...
            width: constants::pixels_to_excel_column_width(constants::DEFAULT_COLUMN_WIDTH),
            custom_width: false,
    ...
    pub(crate) fn set_column_width_and_style(
        ...
        let mut col = Col {
            min: column,
            max: column,
            width: constants::pixels_to_excel_column_width(width),
        ...
        for c in cols.iter_mut() {
            ...
            if min <= column && column <= max {
                if min == column && max == column {
                    c.style = style;
                    c.width = constants::pixels_to_excel_column_width(width);
    ...
    pub fn get_column_width(&self, column: i32) -> Result<f64, String> {
        ...
        for col in cols {
            ...
            if column >= min && column <= max {
                ...
                if col.custom_width {
                    return Ok(constants::excel_column_width_to_pixels(col.width));
                }
                break;
            }
        }
