...
const ITERATION_MODEL_BYTES_MAGIC: &[u8] = b"IRONCALC_ITERATION_V1\0";

#[derive(bitcode::Encode, bitcode::Decode)]
struct PersistedModelWithIteration {
    workbook: Workbook,
...
pub struct Model<'a> {
    ...
    /// Runtime and persisted options for resolving circular formulas.
    pub(crate) iteration_settings: IterationSettings,
    /// Set when the current calculation pass encounters a circular dependency.
    pub(crate) iteration_cycle_detected: bool,
}
...
impl<'a> Model<'a> {
    ...
    fn get_range(&mut self, left: &Node, right: &Node, cell: CellReferenceIndex) -> CalcResult {
        ...
        match (left_result, right_result) {
            (
                ...
                {
                    let first_sheet = left1.sheet.min(right2.sheet);
                    let last_sheet = left1.sheet.max(right2.sheet);
                    let first_row = left1.row.min(right2.row);
                    let last_row = left1.row.max(right2.row);
                    let first_column = left1.column.min(right2.column);
                    let last_column = left1.column.max(right2.column);
    ...
    /// Evaluates an internally-stored English formula in the context of a cell.
    ///
    /// This is intended for workbook features whose formulas are not stored in
    /// cells, such as data validation. Relative references are resolved as if
    /// the formula belonged to (`sheet`, `row`, `column`). The workbook is not
    /// mutated by this operation, although callers should evaluate it first if
    /// they have pending cell changes.
    pub fn evaluate_formula_value_at(
        &mut self,
        formula: &str,
        sheet: u32,
        row: i32,
        ...
        let context_index = CellReferenceIndex { sheet, row, column };
        match self.evaluate_node_in_context(&node, context_index) {
            CalcResult::Number(value) => Ok(CellValue::Number(value)),
            CalcResult::String(value) => Ok(CellValue::String(value)),
            CalcResult::Boolean(value) => Ok(CellValue::Boolean(value)),
            CalcResult::EmptyCell | CalcResult::EmptyArg => Ok(CellValue::None),
            CalcResult::Error { error, message, .. } => {
                Err(format!("Formula evaluation failed ({error}): {message}"))
            }
            CalcResult::Range { .. } => Err("Formula returned a range instead of a scalar".into()),
            CalcResult::Array(_) => Err("Formula returned an array instead of a scalar".into()),
            CalcResult::Lambda(_) => Err("Formula returned a lambda instead of a scalar".into()),
        }
    }

    /// Evaluates an internally-stored English formula as a rectangular value array.
    ///
    /// Range references, defined names, structured table references, spill references and
    /// array-returning formulas are expanded in row-major order. Scalar results are returned as
    /// a one-by-one array. This is primarily intended for list-like workbook features such as
    /// data validation and does not mutate the workbook.
    pub fn evaluate_formula_values_at(
        &mut self,
        formula: &str,
        sheet: u32,
        row: i32,
        ...
        let body = formula.trim().strip_prefix('=').unwrap_or(formula.trim());
        if body.is_empty() {
            return Ok(vec![vec![CellValue::None]]);
        }
        let sheet_name = self.workbook.worksheet(sheet)?.get_name();
        let context_rc = CellReferenceRC {
            sheet: sheet_name,
        ...
        let context_index = CellReferenceIndex { sheet, row, column };
        match self.evaluate_node_in_context(&node, context_index) {
            CalcResult::Number(value) => Ok(vec![vec![CellValue::Number(value)]]),
            CalcResult::String(value) => Ok(vec![vec![CellValue::String(value)]]),
            CalcResult::Boolean(value) => Ok(vec![vec![CellValue::Boolean(value)]]),
            CalcResult::EmptyCell | CalcResult::EmptyArg => Ok(vec![vec![CellValue::None]]),
            CalcResult::Error { error, message, .. } => {
                Err(format!("Formula evaluation failed ({error}): {message}"))
            }
            CalcResult::Range { left, right } => {
                if left.sheet != right.sheet {
                    return Err("Formula returned a three-dimensional range".into());
                }
                let mut values = Vec::new();
                for range_row in left.row.min(right.row)..=left.row.max(right.row) {
                    let mut row_values = Vec::new();
                    for range_column in
                        left.column.min(right.column)..=left.column.max(right.column)
                    {
                        row_values.push(self.get_cell_value_by_index(
                            left.sheet,
                            range_row,
                            range_column,
                        )?);
            ...
            CalcResult::Array(values) => values
                .into_iter()
                .map(|row| {
                    row.into_iter()
                        .map(|value| match value {
                            ArrayNode::Number(value) => Ok(CellValue::Number(value)),
                            ArrayNode::String(value) => Ok(CellValue::String(value)),
                            ArrayNode::Boolean(value) => Ok(CellValue::Boolean(value)),
                            ArrayNode::Empty => Ok(CellValue::None),
                            ArrayNode::Error(error) => {
                                Err(format!("Formula array contains error {error}"))
                            }
                        })
                        .collect::<Result<Vec<_>, _>>()
                })
                .collect(),
            CalcResult::Lambda(_) => Err("Formula returned a lambda instead of values".into()),
        }
    }
            ...
            match original_range {
                ...
                None => {
                    ...
                    let coerced = if array_width == 1 && array_height == 1 {
                        ...
                        // OpenCell patch: 静态分析漏网的数组结果按隐式交集取左上角兕底，
                        // 绝不 panic（服务常驻，单条公式不能拖垮进程）。
                        match self.get_value_from_array(array, 1, 1) {
                            Some(node) => array_node_to_formula_value(node),
                            None => FormulaValue::Error {
                                ei: Error::VALUE,
    ...
    fn validate_iteration_settings(settings: &IterationSettings) -> Result<(), String> {
        if !(1..=32_767).contains(&settings.maximum_iterations) {
            return Err("Maximum iterations must be between 1 and 32767".to_string());
        }
        if !settings.maximum_change.is_finite() || settings.maximum_change < 0.0 {
            return Err("Maximum change must be a finite non-negative number".to_string());
        }
        Ok(())
    }

    /// Returns the workbook's iterative-calculation settings.
    pub fn get_iteration_settings(&self) -> IterationSettings {
        self.iteration_settings.clone()
    }

    /// Sets all iterative-calculation settings after validating Excel-compatible limits.
    pub fn set_iteration_settings(&mut self, settings: IterationSettings) -> Result<(), String> {
        Self::validate_iteration_settings(&settings)?;
        self.iteration_settings = settings;
        Ok(())
    }

    /// Configures iterative calculation for circular formulas.
    pub fn set_iteration_options(
        &mut self,
        enabled: bool,
        maximum_iterations: u32,
        maximum_change: f64,
    ) -> Result<(), String> {
        self.set_iteration_settings(IterationSettings {
            enabled,
            maximum_iterations,
            maximum_change,
        })
                ...
                if let Some(state) = self.cells.get(&key) {
                    match state {
                        CellState::Evaluating => {
                            if self.iteration_settings.enabled {
                                self.iteration_cycle_detected = true;
                                return match &original_cell {
                                    Cell::CellFormula { v, .. } | Cell::ArrayFormula { v, .. } => {
                                        match v {
                                            ...
                                            _ => {
                                                self.get_cell_value(&original_cell, cell_reference)
                                            }
                                        }
                                    }
                                    _ => self.get_cell_value(&original_cell, cell_reference),
                                };
                            }
                            return CalcResult::new_error(
                                Error::CIRC,
    ...
    pub fn from_bytes(s: &[u8], language_id: &'a str) -> Result<Model<'a>, String> {
        if let Some(payload) = s.strip_prefix(ITERATION_MODEL_BYTES_MAGIC) {
            let persisted: PersistedModelWithIteration = bitcode::decode(payload)
                .map_err(|e| format!("Error parsing workbook with iteration settings: {e}"))?;
            return Model::from_workbook_with_iteration(
                persisted.workbook,
                language_id,
                persisted.iteration_settings,
            );
        }
    ...
    /// Creates a model from a workbook and explicit iterative-calculation settings.
    ///
    /// This is the import entry point for formats such as XLSX whose workbook-level
    /// calculation properties include `iterate`, `iterateCount`, and `iterateDelta`.
    pub fn from_workbook_with_iteration(
        workbook: Workbook,
        language_id: &str,
        iteration_settings: IterationSettings,
    ) -> Result<Model<'_>, String> {
        Self::validate_iteration_settings(&iteration_settings)?;
        let mut model = Self::from_workbook(workbook, language_id)?;
        model.iteration_settings = iteration_settings;
        Ok(model)
    }
    ...
    pub fn from_workbook(workbook: Workbook, language_id: &str) -> Result<Model<'_>, String> {
        ...
        let mut model = Model {
            ...
            iteration_settings: Default::default(),
            iteration_cycle_detected: false,
        };
    ...
    /// Stores the flattened calculation value of an OOXML rich string as text.
    ///
    /// Unlike [`Model::set_user_input`] and [`Model::update_cell_with_text`], this method never
    /// interprets a leading `=` as a formula, never parses number-looking text, and does not add
    /// a quote-prefix style. The caller owns the corresponding per-run rich-text metadata.
    pub fn set_rich_text_plain_value(
        &mut self,
        sheet: u32,
        row: i32,
        column: i32,
    ...
    fn formula_values_snapshot(&self) -> Vec<(CellReferenceIndex, FormulaValue)> {
        self.get_all_cells()
            .into_iter()
            .filter_map(|cell| {
                ...
                match self.fetch_cell(reference) {
                    Some(Cell::CellFormula { v, .. }) | Some(Cell::ArrayFormula { v, .. }) => {
                        Some((reference, v.clone()))
                    }
                    _ => None,
                }
            })
    ...
    fn formula_value_converged(
        ...
        match (before, after) {
            (FormulaValue::Number(a), FormulaValue::Number(b)) => {
                a == b || (a - b).abs() <= maximum_change
            }
            _ => before == after,
        }
    }

    fn formula_snapshots_converged(
        before: &[(CellReferenceIndex, FormulaValue)],
        after: &[(CellReferenceIndex, FormulaValue)],
        maximum_change: f64,
    ) -> bool {
        before.len() == after.len()
            && before.iter().zip(after).all(
                |((before_ref, before_value), (after_ref, after_value))| {
                    before_ref == after_ref
                        && Self::formula_value_converged(before_value, after_value, maximum_change)
                },
            )
    }

    /// Runs one workbook calculation pass using the two-phase dynamic-array algorithm.
    ///
    ...
    fn evaluate_pass(&mut self) {
        self.iteration_cycle_detected = false;
    ...
    /// Evaluates the workbook, optionally iterating circular formulas until convergence.
    ///
    /// Iteration is disabled by default, preserving the normal `#CIRC!` result. When enabled,
    /// formula values from the preceding pass seed circular references, and calculation stops
    /// when every formula is stable within `maximum_change` or `maximum_iterations` is reached.
    pub fn evaluate(&mut self) {
        ...
        let settings = self.iteration_settings.clone();
        let mut before = self.formula_values_snapshot();
        for _ in 0..settings.maximum_iterations {
            self.evaluate_pass();
            let after = self.formula_values_snapshot();

            if !self.iteration_cycle_detected
                || Self::formula_snapshots_converged(&before, &after, settings.maximum_change)
            {
                break;
            }
            before = after;
    ...
    /// Returns an internal binary representation of the workbook and calculation settings.
    ///
    /// Workbooks using the default iteration settings retain the legacy raw-workbook encoding.
    /// A versioned envelope is emitted only for non-default settings, and [`Model::from_bytes`]
    /// accepts both encodings.
    ///
    /// See also:
    /// * [Model::from_bytes]
    pub fn to_bytes(&self) -> Vec<u8> {
        if self.iteration_settings == IterationSettings::default() {
            return bitcode::encode(&self.workbook);
        }
        let persisted = PersistedModelWithIteration {
            workbook: self.workbook.clone(),
            iteration_settings: self.iteration_settings.clone(),
        };
        let payload = bitcode::encode(&persisted);
        let mut bytes = Vec::with_capacity(ITERATION_MODEL_BYTES_MAGIC.len() + payload.len());
        bytes.extend_from_slice(ITERATION_MODEL_BYTES_MAGIC);
