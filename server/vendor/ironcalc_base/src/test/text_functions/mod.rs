...
#[test]
fn test_value_uses_legacy_implicit_intersection_for_ranges() {
    let mut model = new_empty_model();
    model._set("A1", "'10");
    model._set("A2", "'20");
    model._set("A3", "'30");
    model._set("B2", "=VALUE(A1:A3)");
    model._set("B4", "=VALUE(A1:A3)");
    model.evaluate();
...
#[test]
fn test_t_broadcasts_over_dynamic_arrays() {
    let mut model = new_empty_model();
    model._set("A1", "=T({\"text\",42;#N/A,TRUE})");
    model.evaluate();
