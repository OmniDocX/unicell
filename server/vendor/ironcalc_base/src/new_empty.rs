...
use crate::{
    ...
    types::{
        DefinedName, Metadata, SheetState, Table, Workbook, WorkbookSettings, WorkbookView,
...
impl<'a> Model<'a> {
    /// Replace the workbook's structured-table registry and rebuild parser
    /// state so newly-created or resized tables are immediately available to
    /// structured-reference formulas without reopening the XLSX file.
    pub fn replace_tables(&mut self, tables: HashMap<String, Table>) {
        self.workbook.tables = tables.clone();
        ...
        self.parsed_formulas.clear();
        self.parsed_defined_names.clear();
        self.cells.clear();
        self.parse_formulas();
